...
import time
from Utils.Parallel_mapping.circuit_tran import fun_cost
from Utils.Parallel_mapping.circuit_tran import shortway
...
def entail(EG, tau, gate):
    ...
    if (u not in tau) or (v not in tau):
        ...
        p = tau.index(gate[1])  # p is the physical qubit that gate[0] occupies
        q = tau.index(gate[2])  # q is the physical qubit that gate[1] occupies

        if (p, q) in EG or (q, p) in EG:  # G is the architecture graph, EG its edge list
            return True
        else:
            return False
...
def unoccupied(tau):
    ...
    随架构修改
    '''
    U = list(p for p in range(67) if tau[p] == 66)
    return U
...
def greedy_solved_gates(C, G, EG, nl, tau):
...
def good_next_mappingx(C, G, EG, nl, tau, QFilter_type):
    ...
    for rho in SWAP3x(C, EG, nl, tau, LD, QFilter_type):
        u = len(rho[0])
        x = greedy_solved_gates(C, G, EG, nl, rho[1])  # x has form [solved_gates]
        if not x:
            continue
        t = len(x) / (
    ...
    if not SG:
        ...
        for rho in SWAP4x(C, EG, nl, tau, LD, QFilter_type):
            ...
            x = greedy_solved_gates(C, G, EG, nl, rho[1])
            if not x:
                continue
            t = len(x) / (
                        3 * u)  # the efficiency rate of rho is defined as (number of solved gates)/(3*(number of swaps used))
    ...
    #以下函数处理相同代价下的路径选择
    # print('交换序列',SG[0][0])
    max = fun_cost.cost(C,SG[0][0],tau,LD,G,wayindex)
...
#相同代价下的比较
    for i in range(len(SG)-1):
        if cost == SG[i+1][1]:
            n = fun_cost.cost(C, SG[i+1][0],tau, LD, G, wayindex)
            if n > max:
                max = n
                path = SG[i+1][0][0]
                taunew = SG[i+1][0][1]
...
def qct(Circ, G, EG, tau, QFilter_type, BB, C):
    ...
    CircLen = len(Circ) #原始线路门数
    l = len(Circ)  # 剩余门数

    nsvg = 0  # the number of executed gates 已经可执行门数
    COST = 0  # the number of auxiliary CNOT gates 插入门数
    taunew = tau[:] #当前映射更新
    cir_remove = Circ[:]
    nr = 0
    C_out = []  # the output circuit 输出线路 包括单门
    # BBnew = BB
    start = time.time()
    # value = 0
    for i in Circ:
        #判断是否是cnot门
        #若不是，直接加入C-out输出线路，并在原线路中删除该门
        #可执行门数加1
        #while nsvg < CircLen:
        if isinstance(i[1], str):
            pos = taunew.index(i[2])
            C_out.append([i[1], pos])
            nsvg += 1
            cir_remove.remove(cir_remove[0])
        #否则判断当前映射下是否满足近邻
        else:
             #可执行门数和原始门数比较
            # first check what gates can be executed by taunew (obtained in the previous round),
            # then remove their indices from L1
            pos1 = taunew.index(i[1])
            pos2 = taunew.index(i[2])
            if (pos1,pos2) in EG or (pos2,pos1) in EG:
                C_out.append([pos1, pos2])
                nsvg += 1
                cir_remove.remove(cir_remove[0])
            else:
                Y = good_next_mappingx(cir_remove, G, EG, nl, taunew, QFilter_type)
        ...
        #     print(taunew)
        #     GSG = greedy_solved_gates(cir_remove, G, EG, nl, taunew)
        #     if not GSG.index(0):
        #         print('首门能被')
        #         for k in range(GSG[0],GSG[len(GSG)-1]+1):
        #             # if C[i] is executed, remove i from L1 and add CNOT gate [p,q] to C_out
        #             if isinstance(Circ[0][1],str):
        #                 C_out.append([Circ[0][1], taunew.index(x)])
        #                 Circ.remove(Circ[0])
        #                 nsvg += 1
        #                 break
        #             else:
        #                 p = taunew.index(Circ[0][1])
        #                 q = taunew.index(Circ[0][2])
        #                 C_out.append([p, q])
        #                 Circ.remove(Circ[0])
        ...
        #
        #     #当前映射不满足首门近邻条件
        #     if not len(cir_remove):
        #         ##        print('We have solved all gates within %s rounds' %(nr+1))
        #         break
        #
        #     nr += 1
        #     Y = good_next_mappingx(Circ, G, EG, nl, taunew, QFilter_type)
        #     path = Y[0]  # a sequence of swaps that transforms the previous mapping into the mapping tuanew below
        #     taunew = Y[1]
        #
        #     cost = len(path) * 3
        ...
        #     pos2 = taunew.index(i[2])
        #     for j in EG:
        #         if (j[0]==pos1 and j[1]==pos2) or (j[0]==pos2 and j[1]==pos1):
        #             C_out.append([pos1, pos2])
        #             nsvg += 1
        #             cir_remove.remove(cir_remove[0])
        #             break
...
if __name__ == '__main__':
    from Q66 import q66
    cir = [['h', 6], [4, 6], ['tdg', 6], [3, 6], ['t', 6], [4, 6], ['tdg', 6], [3, 6], ['t', 6], ['t', 4], [3, 4],
            ['t', 3], ['x', 3], ['tdg', 4], [3, 4], [4, 6], ['tdg', 6], [3, 6], ['t', 6], [4, 6], ['tdg', 6], [3, 6]]
    ...
    EG = nx.edges(G)  # 拓扑图边
    tau = [66, 66, 66, 66, 66, 66, 66, 11, 1, 10, 66, 66, 66, 0, 9, 66, 66, 66, 66, 7, 5, 3, 66, 66, 66, 6, 4, 66, 66, 66, 66, 66, 13, 12, 66, 66, 66, 66, 2, 66, 66, 66, 66, 66, 66, 8, 66, 66, 66, 66, 66, 66, 66, 66, 66, 66, 66, 66, 66, 66, 66, 66, 66, 66, 66, 66, 66]
    QFilter_type = '2x'
    BB = {13: 32, 12: 33, 2: 38, 11: 7, 1: 8, 10: 9, 0: 13, 9: 14, 8: 45, 7: 19, 5: 20, 3: 21, 6: 25, 4: 26}
    C = [[1, 4, 6], [3, 3, 6], [5, 4, 6], [7, 3, 6], [10, 3, 4], [14, 3, 4], [15, 4, 6], [17, 3, 6], [19, 4, 6], [21, 3, 6]]
    cout = []
    cout = qct(cir,G,EG,tau,QFilter_type,BB,C)
    print(cout)