            ...
            if isinstance(g, (ZPhase, XPhase)):
                ...
                if g.phase.denominator == 2:
                    if g.phase.numerator % 4 == 1:
                        ...
                        c.add_gate("S", g.target, adjoint=True)
    ...
    for gate in gates:
        ...
        if len(l) == 2 and l[1].find('nocontrol') != -1:  # no controls
            if gname == "H":
                ...
                c.add_gate("NOT", target)
            elif gname == "Z":
                c.add_gate("Z", target)
            elif gname == "S":
                c.add_gate("S", target, adjoint=adjoint)
            elif gname == "T":
                c.add_gate("T", target, adjoint=adjoint)
            else:
        ...
        if ctrls.find(',') != -1:
            ...
            if gname == "not":
                c.add_gate("TOF", ctrl1, ctrl2, target)
            elif gname == "Z":
                c.add_gate("CCZ", ctrl1, ctrl2, target)
            for t in nots: c.add_gate("NOT", t)
            continue
        elif ctrls.find('+') == -1:
        ...
        ctrl = int(ctrls[1:])
        if gname == "not":
            c.add_gate("CNOT", ctrl, target)
        elif gname == "Z":
            c.add_gate("CZ", ctrl, target)
        elif gname == "X":
            c.add_gate("CX", ctrl, target)
        else:
    ...
    def parse_command(self, c, registers):
        ...
        for j in range(dim):
            ...
            if name in ("x", "z", "s", "t", "h", "sdg", "tdg"):
                if name in ("sdg", "tdg"):
                    g = qasm_gate_table[name](argset[0], adjoint=True)
                else:
            ...
            if name.startswith("rx") or name.startswith("rz"):
                ...
                if name.startswith('rx'):
                    g = XPhase(argset[0], phase=phase)
                else:
                    g = ZPhase(argset[0], phase=phase)
                gates.append(g)
                continue
            if name in ("cx", "CX", "cz"):
